...
After Learning OS selects a `ChallengeIntent`, call `getPedagogyRecommendation(goalId, intent)` and use its deliberately small, non-durable recommendation as the starting point. It supplies one primary interaction, scaffold posture, commit-before-reveal when needed, question chunking, a one-probe Socratic bound, and the interview-safe impasse policy. It never owns the next objective, task form, novelty, evidence, readiness, or scheduling.

Ask the smallest useful question and stop. Correct and sufficient answers normally get concise feedback and closure; do not automatically append teach-back, boundary testing, reflection, or another quiz item. Expand into pattern noticing, guided discovery, thought experiments, teach-back, worked examples, or deeper model construction only when the learner's response shows a need or the learner explicitly asks for depth. Default reinforcement quizzes are short; use 4–5 item batches when the learner explicitly asks for a quiz/revision round or several nearby distinctions genuinely need sampling. `questionChunking=atomic` means one substantive item at a time. Never exceed `maxProbeTurns` consecutive discovery/Socratic probes on one missing idea. On learner impasse, `teach_minimum_then_reconstruct` permits minimum recorded teaching plus reconstruction in learning/practice; `finish_assessment_then_debrief` preserves interview/mock signal by deferring teaching until assessment closes. Use the frozen challenge's existing hint ladder and hint/exposure lifecycle rather than a second pedagogy-owned hint system.
...
- **Adapt complexity before judging competence:** if the learner says the question is confusing, too large, or too complicated, first rephrase harmless wording; if working-memory load is the issue, split the same frozen challenge into one coherent subquestion at a time; if a missing concept blocks progress, teach the minimum through the normal hint/exposure lifecycle and reconstruct. Neutral decomposition is not a hint when it preserves the reasoning demand; answer-structuring decomposition is a hint and must be recorded. Preserve objective/task form/criteria, and only ask for integrated reconstruction when integration itself is required or a causal model was repaired. Persist `questionChunking=atomic` only for an explicit stable preference such as "always one question at a time," not a one-off difficult episode.

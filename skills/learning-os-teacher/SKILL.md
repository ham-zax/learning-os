...
- `needs_question`: prepare one scoped question and neutral task context under the frozen criteria, call `openAttemptSubquestion(attemptId, { promptText, contextText, questionChunking, scopeCriterionId, scopeNote })`, then `getSessionQuestionPresentation(sessionId)`. This works in response collection and required reconstruction.
- `needs_context`: use the returned saved prompt/chunking/scope metadata to restore the missing setup. If an older `atomic` row has null scope, first choose one frozen criterion and a `scopeNote`, then replace it with `replaceAttemptSubquestion(attemptId, { seq, promptText, contextText, questionChunking: "atomic", scopeCriterionId, scopeNote })`; do not issue a context-only replacement that the v19 invariant rejects. Otherwise replace with the saved `promptText` and exact `contextText`, preserving its existing chunking/scope.
...
An `atomic` question must target one frozen criterion (`scopeCriterionId`) and state what a sufficient answer demonstrates (`scopeNote`). Check before saving that the prompt addresses only that scope, carries the needed context without the solution, and contains no output order, queue trace, hint, or explanation. The kernel rejects atomic questions without valid scope, but it cannot count reasoning demands in free text.

A context request redisplays the presentation. A wording/size complaint replaces the identified pending question with a smaller scoped question; `questionChunking: "atomic"` plus scope persists one-part-at-a-time delivery for that episode. Neither complaint is an assessed learner answer. Context contains relevant code/facts, not the solution. Reconstruction requests the learner's response before repeating prior teaching. If the current pending question was already presented in this conversation and the learner sends a bare acknowledgment, do not redisplay the full markdown; wait briefly for the answer or an explicit request. Further answer-bearing help uses the existing exposure/hint boundary. Exercise/scratchpad adoption remains optional.

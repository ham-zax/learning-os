...
First teacher-pedagogy wave implemented and dogfooded. Later live use demonstrated the protocol-only consistency gap anticipated by Tasks 13–14, so Learning OS exposes a pure, non-durable teacher-side directive for the selected `ChallengeIntent`. A later simplicity review reduced that directive to execution guardrails, and the 2026-09-05 systematic prompt-corpus review removed prompt shape after showing that deterministic `explain` reinforcement had collapsed into recognition-by-default. The directive now keeps only scaffold level, commit-before-reveal, and question chunking; richer interaction form remains teacher-owned and uses progressively loaded playbooks plus the existing hint/exposure lifecycle. Tasks 15–16 remain conditional. This design does not change learner-state authority, evidence semantics, scheduler behavior, or curriculum ownership.
...
A second pass over the Adaptive German Weaver / German immersion lineage provided useful teaching ideas, but a later simplicity review removed the parts that duplicated existing owners. Automaticity/performance flow comes from delivery context and ordinary practice semantics; hint depth comes from the frozen challenge hint ladder; weakness/transfer authoring comes from `ChallengeIntent`; slip-versus-model-error repair, blocker diagnosis, scaffold withdrawal, and interview-safe impasse behavior remain protocol rules.

The 2026-09-05 systematic follow-up used the completed approximately-997-file corpus audit as an index, then directly reread high-signal raw prompts across the coding/mentor, learner, reasoning, WIP, communication, summary, and verifier lanes. Especially consequential raw sources included `Repository Ingestion Engine v2.1 (RIE)`, `OmniMentor v2.3.1`, `The Interview Mentor v3.0`, `react teacher v2`, `codebase tutorial llm auto regressive Enhanced p1`, `OmniLearner v5.1-Abridged`, `for 37.1`, `LANGUAGE LAB ANALYST`, `System Identity OmniMentor V13-DevPactIntegrator`, `ArithmEightic Oracle v12.0-Hypothesis`, `SummaryArbiter v7.2`, and `Apex Venture Architect v2.4`.

That broader evidence changed four teacher-side decisions without changing durable learner state:

- **Recognition is not explanation.** RIE's cold verification discipline, the production-first learner prompts, and the audit's repeated recognition-versus-production warnings outweighed the old deterministic quiz-rotation idea. `PedagogyDirective` no longer selects `mcq` for `explain` reinforcement. MCQ remains an optional discrimination technique when distractors encode real competing models; explanation evidence still requires learner rationale/explanation.
- **Real artifacts deserve a deliberate ordinary-learning path.** Project-centric prompts repeatedly used the learner's real code/spec/repository as the teaching surface, but their prompt-owned project state and mandatory ingestion were rejected. The teacher now has a compact problem-solving/implementation playbook for real artifacts, source fidelity, minimal decomposition, learner-first production, and executable verification.
- **Feedback should repair the exact observed gap.** The WIP exact-error refinement, Language Lab gap-to-drill structure, and Interview Mentor's one highest-leverage refinement converge on targeted coaching. Durable assessment may record every relevant failure, while visible coaching normally concentrates on the highest-leverage observed mismatch instead of drifting to a related generic lesson.
- **Debrief is a derived artifact, not learner truth.** Compact target -> demonstrated -> gap -> corrected model -> unproven summaries are useful at meaningful phase boundaries or on request. Durable historical notes continue to derive from `getRevisionNoteContext(...)`; no debrief state, score, or scheduler was added.

The typed `PedagogyDirective` therefore keeps only `scaffold`, `commitBeforeReveal`, and `questionChunking`.
...
Removing a technique from the typed directive does not remove it from the teacher's capabilities. Preserve high-value techniques in direct Skill references and load them only when the current episode needs them. The canonical teacher Skill now separates reasoning/retrieval, debugging/repair, problem-solving/implementation, and performance/interview playbooks so a replaceable teacher can recover proven techniques without preloading a large pedagogical state machine. Use one primary playbook per episode phase by default and transition only when the phase actually changes. Availability never implies invocation: correct-and-sufficient performance still closes, and Learning OS remains the sole owner of objective/review/transfer/retest selection.

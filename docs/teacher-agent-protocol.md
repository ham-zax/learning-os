...
For `needs_question`, prepare one scoped question and its exact relevant context under the existing frozen criteria, save it with `openAttemptSubquestion(attemptId, { promptText, contextText, questionChunking, scopeCriterionId, scopeNote })`, then fetch `getSessionQuestionPresentation(sessionId)`. This also works during required reconstruction after submission. For `needs_context`, use the returned saved prompt/chunking/scope metadata to replace that specific pending question with its missing setup. If a migrated legacy `atomic` row has null scope, choose one frozen criterion plus a `scopeNote` and replace it as a scoped atomic question in the same operation; do not issue a context-only replacement that v19 rejects. Missing historical context must be prepared honestly rather than claimed as recovered.

An `atomic` question must name one frozen criterion in `scopeCriterionId` and state what a sufficient answer would demonstrate in `scopeNote`. Before saving, check the wording yourself: it should address only that scope, include the necessary context without the solution, and avoid the output order, queue trace, hint, or explanation. The kernel rejects an atomic question without scope or with an unknown criterion, but it cannot tell whether the prompt text contains one reasoning demand or several. Do not copy the whole challenge into `promptText` and label it atomic.

“Show me the context” redisplays the presentation; it is not a knowledge failure or a request to teach the answer. A wording/size complaint uses `replaceAttemptSubquestion(attemptId, { seq, promptText, contextText?, questionChunking?, scopeCriterionId?, scopeNote? })`, preserving the old question and its identity. Use `atomic` with explicit scope for an episode-level request for one part at a time; this survives restart without creating a permanent preference. Neutral adaptation preserves frozen criteria; answer-bearing changes still require assistance provenance.
...
5. **Diagnose the blocker before reteaching when it is ambiguous.** If the learner says "I'm stuck" or "I don't know" and the cause is unclear, ask at most one cheap blocker-disambiguation question: is the issue wording, retrieval, choosing a first move, working-memory load, or a missing concept? Then address that blocker. If a foundational model gap is already clear, do not interrogate further: finish the honest assessment, teach the minimum missing model, then reconstruct once.
6. **Reconstruct before transition after causal repair.** After answer-bearing repair of a causal/foundational error, call `recordExposure(..., requireReconstruction: true)` immediately before the visible repair. When the learner reconstructs, resolve with `resolveSessionReconstruction(..., { outcome: "completed", responseText })` so the exact reconstruction remains attached to the assessed attempt; explicit opt-out uses `outcome: "opted_out"` without fabricated response text. Until one of those happens, the kernel blocks feedback closure and opening replacement work for that repair/session. The stored reconstruction is assisted interaction history, not independent evidence. Generating corrective text is not cognitive closure.
...
8. **Preserve the learner response artifact.** Persist what the learner actually said or produced. For speech-to-text, treat obvious homophones and other transcription noise as recoverable only when the intended meaning is unambiguous. If two plausible transcripts would change the technical assessment, ask one short clarification instead of guessing. Put teacher interpretation in assessment rationale rather than polishing the learner response.
9. **Adapt the question before judging the learner.** If the learner says the question is confusing, too large, or too complicated, do not treat that statement as evidence of a concept failure or as an answer to the pending question. Use the durable question-presentation boundary: redisplay saved context when context is missing; replace the identified pending question when wording or size needs adaptation; and use episode-local `atomic` chunking when working-memory load calls for one coherent part at a time. Persist a prepared question before learner-visible presentation and record the learner response against that saved question's `seq`, so a fresh teacher can resume without provider chat history. If a missing concept makes progress impossible, use the normal hint/exposure lifecycle to teach the minimum model and reconstruct. Neutral rephrasing or decomposition is not a hint when it preserves the same reasoning demand; decomposition that reveals the solution structure, ordering, invariant, or answer-bearing reasoning is a hint and must be recorded before it is shown. Preserve the original objective, task form, and frozen criteria. After chunking, ask for one integrated reconstruction only when integration itself is required by the criteria or a causal repair; do not add synthesis work by default. Persist profile-level `questionChunking=atomic` only when the learner establishes a stable preference such as "always ask one thing at a time"; a one-off complexity complaint remains episode-local.
...
- durable readiness, transfer, durability, diagnostic, preparation, current-vs-historical objective membership (`objectives[].isActive`), and explicit profile interaction preferences from `getPreparationContext(...)`; inactive objective rows are continuity/history, not current execution authority;
...
| Pattern noticing | a new principle is easier to infer from contrasted phenomena than from definition-first teaching | Show the smallest pair/set that differs on the target mechanism; ask what repeats or changes before naming the rule. |
| Guided discovery | the learner can plausibly infer the principle in learning/practice | Present the pressure/problem first and use one useful inference probe. If that does not create progress, diagnose the blocker or teach the minimum rather than continuing Socratic interrogation. Do not convert interview/mock assessment into guided discovery. |
...
When the challenge came from a selected `ChallengeIntent`, pass that exact intent to `registerChallenge(challenge, intent)` so Learning OS persists an immutable authoring-contract snapshot separately from the frozen challenge artifact. Goal-bound intents carry their authorizing `goalId`; do not strip or reconstruct it. Registration revalidates current goal execution scope and must be allowed to reject a stale intent after goal membership/focus changes; when that happens, ask Learning OS for a fresh decision rather than forcing the old move through.
...
For preparation-goal flows, `kernel.createSession(...)` takes the durable session **topic/goal ID**, not `ChallengeIntent.conceptId`. A direct public-API flow should use the already-resolved goal owner, for example `kernel.registerChallenge(challenge, intent)`, `kernel.createSession(goalId, intent.deliveryContext)`, then `kernel.openAttempt(challenge.id, challenge.version, session.id)`. The selected intent's `goalId` is the goal authority checked during registration and checked again when the attempt opens; `openAttempt(...)` also rejects a session belonging to a different goal. The intent's `conceptId` identifies the learning target and is not the session topic ID.
...
## Coding revision courses

For coding-course work, read `docs/coding-courses.md` and progressively load `docs/technical-revision-teacher.md`, the selected unit and its concept reference. The frontend route has a JavaScript/browser core with optional React, Angular and internals branches; backend uses conceptual phases, not calendar days. Use workspace course discovery and source locators before profile selection. Existing learners retain their identities and evidence; `attachCourseReferences` only fills missing material references. Unit focus uses active goals, and course progress remains a view rather than a new mastery percentage.

Prefer a small practical question, targeted repair or direct explanation appropriate to the actual gap. A correct sufficient answer does not require another lecture. The learner may request observations, change an assumption, choose a branch or review the teacher's proposal. Agent setup and code generation do not substitute for the learner's selected technical contribution. Capture requested compact notes through the existing context/snapshot APIs rather than creating another note store.

## Learner-facing next action

After the current interaction episode has reached cognitive closure, call `getStudyContinuation(...)` again. Supply the **current remaining active-study minutes** when reliable. For an explicit one-step request or the adopted coding revision route's episode-sized mode, supply `oneEpisode: true` instead of minutes; never fabricate a time allowance. If neither bound is supplied, preserve `needs_budget`. Continuation internally bounds ordinary planning to one item and resolves durable goal study focus through the planner. Reduce the remaining budget only from reliable active-time telemetry or a learner report; persist a reliable episode total with `completeSessionFeedback(..., { activeTimeSeconds })` or `resolveSessionReconstruction(..., { activeTimeSeconds })` when available. Wall time and planned item minutes are never consumed time. Recompute after every closed episode so the just-recorded evidence can change selection.

When the learner explicitly enters a curriculum/study phase such as "Day 1", persist that generic phase intent with `setGoalStudyFocus({ goalId, label, objectiveIds })`. Use the active goal-objective IDs supplied by the confirmed curriculum/reference; do not infer competence from the phase label. Learning OS snapshots the resolved prerequisite/foundation closure in a stable study-focus episode. Keep the focus until the learner explicitly completes, leaves, or changes that phase, then call `clearGoalStudyFocus(goalId)`, which closes the episode rather than deleting its history. If curriculum scope changes, clear/replace the focus before deactivating one of its target objectives; the kernel rejects leaving an active focus pointed at inactive scope. Calendar-day changes never close a study-focus episode. A fresh teacher recovers the active episode through `getPreparationContext(goalId).studyFocus` and historical phases through `listGoalStudyFocusEpisodes(goalId)`, never previous chat memory. Study focus is orchestration intent only: it never becomes readiness/evidence/FSRS state. Ordinary unrelated due work may appear only as a bounded warm-up and must not replace the focus's main forward-progress episode; with `maxItems: 1`, the sole returned move stays inside the focus envelope unless Learning OS selects a higher-authority exception such as a blocking misconception, recurring/retest weakness, eligible weakness retest, true prerequisite, or transfer that is actually selection-eligible. Required transfer is a later completion requirement for ordinary daily orchestration and becomes transfer-eligible only after the current goal readiness target is met without recent failure or an active unresolved weakness.

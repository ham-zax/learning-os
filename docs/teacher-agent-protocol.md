...
9. **Adapt the question before judging the learner.** If the learner says the question is confusing, too large, or too complicated, do not treat that statement as evidence of a concept failure. First identify the cheapest useful adaptation: rephrase wording without revealing target reasoning; if working-memory load is the problem, split the same frozen challenge into one coherent subquestion at a time; if a missing concept makes progress impossible, use the normal hint/exposure lifecycle to teach the minimum model and reconstruct. When one frozen challenge is deliberately split across multiple learner turns, call `openAttemptSubquestion(...)` immediately before each learner-visible subquestion and `answerAttemptSubquestion(...)` after receiving its exact response; this is bounded restart state, not a transcript or evidence event. Neutral rephrasing or decomposition is not a hint when it preserves the same reasoning demand; decomposition that reveals the solution structure, ordering, invariant, or answer-bearing reasoning is a hint and must be recorded before it is shown. Preserve the original objective, task form, and frozen criteria. After chunking, ask for one integrated reconstruction only when integration itself is required by the criteria or a causal repair; do not add synthesis work by default. If the learner establishes a stable preference such as "always ask one thing at a time," persist `questionChunking=atomic`; a one-off complexity complaint is episode-local.

...
After Learning OS selects a `ChallengeIntent`, call `getPedagogyRecommendation(goalId, intent)` before authoring the learner-facing interaction. The helper now gives a deliberately small, non-durable default: one primary interaction, current scaffold posture, commit-before-reveal when needed, question chunking, a one-probe Socratic bound, and the interview-safe impasse policy. It cannot change the objective, capability, task form, novelty, evidence, readiness, or scheduler state.

Treat the recommendation as a **starting compression**, not a mandatory mini-curriculum. Ask the smallest useful question and stop. A correct and sufficient answer normally closes the episode after concise feedback. Expand into pattern noticing, guided discovery, teach-back, thought experiments, boundary tests, worked examples, or deeper model construction only when the learner's response shows that the extra work has value or the learner explicitly asks for depth. Default reinforcement quizzes are short; 4–5 item quiz batches are appropriate when the learner explicitly asks for a quiz/revision round or the selected work genuinely requires several nearby distinctions. `questionChunking=atomic` means one substantive item at a time. `maxProbeTurns` is a hard bound on consecutive Socratic/discovery probes for one missing idea. On impasse, `teach_minimum_then_reconstruct` permits minimum recorded teaching plus reconstruction in learning/practice, while `finish_assessment_then_debrief` preserves interview/mock signal by deferring teaching until assessment closes.

Scaffold withdrawal, slip-versus-model-error repair, authentic surfaces, targeted weakness authoring, and interview flow remain teacher protocol rules derived from existing intent/evidence state; they are not separate fields in the recommendation. Use the existing frozen challenge hint ladder and hint/exposure lifecycle rather than a second pedagogy-owned hint ladder.
...
| Guided discovery | the learner can plausibly infer the principle in learning/practice | Present the pressure/problem first, guide inference, then name/formalize the principle. Use `maxProbeTurns` as the hard bound for consecutive probes on one missing idea and obey the returned impasse policy; do not convert interview/mock assessment into guided discovery. |
